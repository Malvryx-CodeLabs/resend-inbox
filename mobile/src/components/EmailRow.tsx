...
export function EmailRow({ email, onPress }: EmailRowProps) {
  ...
  return (
    <Pressable
      onPress={onPress}
      className="border-b border-line bg-paper px-5 py-4 active:bg-mist"
    >
      <View className="flex-row items-start justify-between gap-3">
        <View className="min-w-0 flex-1 gap-1">
          <View className="flex-row items-center gap-2">
            {isOutbound ? <ArrowUpRight size={14} color="#2f6fbd" /> : null}
        ...
        <View className="items-end gap-2">
          ...
          {email.attachments.length > 0 ? <Paperclip size={14} color="#7a827b" /> : null}
        </View>
      </View>
    </Pressable>
  );
